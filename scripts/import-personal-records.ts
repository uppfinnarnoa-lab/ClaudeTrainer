/**
 * Import Noa's personal records from the spreadsheet image.
 * Skips records that already exist (same distanceM + date).
 * Run: pnpm tsx scripts/import-personal-records.ts
 */
...
function t(mm: number, ss: number) { return mm * 60 + ss; }
...
async function main() {
  ...
  for (const r of records) {
    ...
    if (existingSet.has(key)) {
      console.log(`SKIP  ${r.distance} ${r.date} (already exists)`);
      skipped++;
      continue;
    }
